...
install starts by defining or importing a criteria profile, then screens
...
1. CONFIGURABLE CRITERIA
   Every review is a "criteria profile" (see criteria_profiles.py) that can be
   created from a file, from pasted text, or by editing an existing profile.
   The system prompt, the JSON output schema, the repository columns, and the
   decision rules are all rendered from the active profile — nothing about the
   criteria is hardcoded in the app.
...
# Repository columns, the analysis call, and record construction are shared
# with the web front end — see screening.py.
CSV_BASE_COLUMNS = screening.CSV_BASE_COLUMNS
BATCH_CSV_COLUMNS = screening.BATCH_CSV_COLUMNS
analyze_paper = screening.analyze_paper
_pid_sort_key = screening.pid_sort_key
...
def ensure_repo():
    screening.Repository(REPO_DIR).ensure()


def load_repository():
    return screening.Repository(REPO_DIR).load()
...
def save_to_repository(entry: dict):
    screening.Repository(REPO_DIR).save(entry)
...
class PaperScreenerApp(tk.Tk):
    ...
    def _load_active_profile(self) -> dict:
        pid = self.settings.get("active_profile_id") or ""
        prof = cp.load_profile(REPO_DIR, pid) if pid else None
        if not prof:
            # No profile created yet (or the saved one is gone). Point at the
            # in-memory placeholder rather than seeding anything to disk —
            # triageQ ships with no criteria of its own.
            self.settings["active_profile_id"] = ""
            save_settings(self.settings)
            return json.loads(json.dumps(cp.EMPTY_PROFILE))
    ...
    def _build_batch_tab(self):
        outer = tk.Frame(self.tab_batch, bg=PALETTE["bg"])
        ...
        self._batch_log = scrolledtext.ScrolledText(
            log_card, font=("Courier New", 9), height=18,
            bg=PALETTE["console_bg"], fg=PALETTE["console_fg"],
            bd=0, padx=16, pady=12, wrap="word")
    ...
    def _build_about_card(self, f):
        inner = self._card(f, label="About", pad=(20, 16))
        ...
        tk.Label(inner,
                 text=(
                     "This application was developed with AI assistance (Claude Sonnet 5). "
                     "It is experimental software intended for initial triage only; "
                     "recommendations should be verified by a human reviewer. triageQ is "
    ...
    def _refresh_profile_widgets(self):
        ...
        if not p.get("profile_id"):
            meta = "No criteria profile yet — create one above to start screening."
            self._profile_meta_label.config(text=meta)
            name_line = "No criteria profile yet"
            self._single_profile_label.config(text=name_line)
            self._batch_profile_label.config(text=name_line)
            self._header_profile_label.config(text="")
            return
    ...
    def _build_repo_entry(self, paper_id: str, result: dict,
        ...
        None for url / pdf_source / pdf_url means "whatever the Single Paper tab
        currently holds"; batch runs pass their own per-row values.
        """
        return screening.build_repo_entry(
            self.active_profile, paper_id, result, basis=basis,
            url=(self._url_var.get().strip() if url is None else url),
            file_path=file_path,
            pdf_source=(self._current_pdf_source if pdf_source is None else pdf_source),
            pdf_url=(self._current_pdf_url if pdf_url is None else pdf_url),
    ...
    def _validate_ready(self, need_pdf=True):
        if not self.active_profile.get("profile_id"):
            messagebox.showwarning(
                "No Criteria Profile",
                "triageQ has no built-in criteria — create or import a profile in the "
